    ...
    <main id="main-content" style={{ maxWidth:600, margin:"0 auto", padding:"60px 20px 80px" }}>
      <header style={{ textAlign:"center", marginBottom:40 }}>
        ...
        <h1 style={{ fontSize:28, fontWeight:800, color:C.textPrimary,
          letterSpacing:"-.5px", marginBottom:8 }}>Upload a Document</h1>
      ...
      <aside aria-label="File requirements" style={{ marginTop:14, padding:"12px 16px",
        background:"rgba(18,18,26,.7)", border:`1px solid ${C.border}`,
        ...
        <span style={{ fontSize:13, color:C.textMuted, flex:1 }}>
          <strong style={{ color:"#cbd5e1" }}>Accepted:</strong> PDF, PNG, JPG, JPEG, WEBP · Max 5 MB
        </span>
        {!isAuthenticated && (
          <span style={{ fontSize:12, color:C.textMuted }}>
            <Link to="/register" style={{ color:C.info, fontWeight:600 }}>Sign up</Link> to save history
          </span>
        )}
      </aside>
      ...
      {!isAuthenticated && (
        <div style={{ marginTop:24, padding:"16px 18px",
          ...
          <Sparkles size={16} color={C.info} aria-hidden="true" style={{ flexShrink:0, marginTop:1 }} />
          <div>
            ...
            <Link to="/register" style={{ display:"inline-flex", alignItems:"center", gap:5,
              marginTop:10, fontSize:13, fontWeight:600, color:C.info, textDecoration:"none" }}>
              Get started free →
            </Link>
          </div>
        </div>
