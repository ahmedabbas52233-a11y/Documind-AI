...
export default function AuthPage() {
  ...
  const onSubmit = async (e) => { e.preventDefault(); const err=validate(); if(err){setValErr(err);return;} const ok=isLogin?await login(form.email,form.password):await register(form.email,form.password); if(ok) navigate("/upload"); };
  const displayErr = authErr || valErr;
  const pwChecks = checks(form.password);
  const score = pwChecks.filter(c=>c.ok).length;
  const strength = SM[score-1];
  const iStyle = { width:"100%", padding:"11px 40px", background:"rgba(18,18,26,.8)",
    border:`1px solid ${C.border}`, borderRadius:9, color:C.textPrimary,
    fontSize:14, outline:"none", transition:"border-color .15s, box-shadow .15s", boxSizing:"border-box" };
  const iconStyle = { position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color:C.textMuted, pointerEvents:"none" };
  return (
    <main id="main-content" style={{ minHeight:"100vh", display:"flex",
      alignItems:"center", justifyContent:"center", padding:"32px 16px", background:C.bg }}>
      <div style={{ width:"100%", maxWidth:400 }}>
        <Link to="/" aria-label="DocuMind AI home"
          style={{ display:"flex", alignItems:"center", gap:9, textDecoration:"none",
            justifyContent:"center", marginBottom:28 }}>
          <div aria-hidden="true" style={{ width:36, height:36, borderRadius:9,
        ...
        <div style={{ ...cardBase, padding:28,
          ...
          <h1 style={{ fontWeight:800, fontSize:21, color:C.textPrimary, textAlign:"center", marginBottom:4 }}>
            {isLogin?"Welcome back":"Create your account"}
          </h1>
          <p style={{ fontSize:13, color:C.textMuted, textAlign:"center", marginBottom:26 }}>
            {isLogin
              ? <><span>No account? </span><Link to="/register" style={{ color:C.info, fontWeight:600 }}>Sign up free →</Link></>
              : <><span>Already have one? </span><Link to="/login" style={{ color:C.info, fontWeight:600 }}>Sign in →</Link></>}
          </p>

          <form onSubmit={onSubmit} noValidate>
            <div style={{ marginBottom:14 }}>
              <label htmlFor={emailId} style={{ display:"block", fontSize:12, fontWeight:600,
                color:"#cbd5e1", marginBottom:6, textTransform:"uppercase", letterSpacing:".05em" }}>
                Email address
              </label>
              <div style={{ position:"relative" }}>
                ...
                <input id={emailId} name="email" type="email" autoComplete="email" required
                  ...
                  aria-invalid={displayErr?"true":"false"} style={iStyle} />
              </div>
            </div>

            <div style={{ marginBottom:18 }}>
              <label htmlFor={passId} style={{ display:"block", fontSize:12, fontWeight:600,
                color:"#cbd5e1", marginBottom:6, textTransform:"uppercase", letterSpacing:".05em" }}>
                Password
              </label>
              <div style={{ position:"relative" }}>
                ...
                <button type="button" onClick={()=>setShowPw(!showPw)}
                  ...
                  style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)",
                    background:"none", border:"none", cursor:"pointer", color:C.textMuted, padding:4, borderRadius:4 }}>
                  ...
                  <ul aria-label="Password requirements"
                    ...
                    {pwChecks.map(({ label, ok }) => (
                      <li key={label} style={{ fontSize:11, display:"flex", alignItems:"center", gap:4,
                        ...
                        <CheckCircle size={10} color={ok?C.success:"#334155"} aria-hidden="true" />
            ...
            {displayErr && (
              <div id={errId} role="alert" style={{ display:"flex", gap:8, padding:"10px 12px",
                marginBottom:16, background:C.errorBg, border:`1px solid ${C.errorBorder}`,
            ...
            <button type="submit" disabled={loading} aria-busy={loading}
              style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center",
                gap:7, padding:"12px 20px", borderRadius:9, background:C.grad, color:"#fff",
                border:"none", cursor:loading?"not-allowed":"pointer", fontWeight:700,
                fontSize:14, fontFamily:"inherit", opacity:loading?.65:1,
          ...
          <div style={{ marginTop:20, paddingTop:20, borderTop:`1px solid ${C.border}`, textAlign:"center" }}>
            <span style={{ fontSize:13, color:C.textMuted }}>
              Just want to try?{" "}
              <Link to="/upload" style={{ color:C.info, fontWeight:600 }}>Analyse as guest →</Link>
            </span>
          </div>
        </div>
      </div>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} input:focus{border-color:rgba(99,102,241,.55)!important;box-shadow:0 0 0 3px rgba(99,102,241,.1)}`}</style>
    </main>
  );
}
