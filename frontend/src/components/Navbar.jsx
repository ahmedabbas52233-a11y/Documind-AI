...
export default function Navbar() {
  ...
  const navLink = (to, Icon, label) => (
    <Link to={to} role="menuitem" aria-current={isActive(to) ? "page" : undefined}
  ...
  return (
    <nav aria-label="Main navigation" style={{
      ...
      <div style={{ maxWidth:1160, margin:"0 auto", padding:"0 24px", height:56,
        ...
        <Link to="/" aria-label="DocuMind AI — Home"
          ...
          <span style={{ fontWeight:800, fontSize:17, color:C.textPrimary, letterSpacing:"-.4px" }}>
          ...
          {isAuthenticated ? (
            <>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <div aria-hidden="true" style={{ width:26, height:26, borderRadius:"50%",
                  background:C.grad, display:"flex", alignItems:"center", justifyContent:"center",
                  fontSize:11, fontWeight:700, color:"#fff" }}>
                  {user?.email?.[0]?.toUpperCase()}
                </div>
                <span style={{ fontSize:12, color:C.textMuted, maxWidth:150,
                  overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={user?.email}>
                  {user?.email}
                </span>
                <button onClick={handleOut} aria-label="Sign out"
                  style={{ background:"none", border:"none", cursor:"pointer",
                    color:C.textMuted, padding:5, borderRadius:6, display:"flex" }}>
            ...
            <>
              <Link to="/login" style={{ padding:"6px 14px", borderRadius:7, fontSize:13,
                fontWeight:500, color:C.textMuted, textDecoration:"none",
                border:`1px solid ${C.border}` }}>Sign In</Link>
              <Link to="/register" style={{ padding:"7px 16px", borderRadius:7, fontSize:13,
                fontWeight:600, background:C.grad, color:"#fff", textDecoration:"none", marginLeft:4 }}>
                Get Started
              </Link>
            </>
          )}
          ...
          {isAuthenticated ? (
            ...
            <>
              <Link to="/login" role="menuitem" onClick={() => setOpen(false)}
                style={{ display:"block", padding:"8px 12px", color:C.textMuted,
                  textDecoration:"none", fontSize:14, fontWeight:500, marginBottom:6 }}>
                Sign In
              </Link>
