...
export default function FileDropZone({ onUploadSuccess, onUploadError }) {
  ...
  const validate = useCallback((f) => {
    ...
    if (!ALLOWED.includes(f.type)) {
      const m = "Unsupported file type. Use PDF, PNG, JPG, or WEBP.";
      setLocalErr(m); onUploadError(m); return false;
    }
    if (f.size > 5 * 1048576) {
      const m = `File too large (${fmt(f.size)}). Max 5 MB.`;
      setLocalErr(m); onUploadError(m); return false;
    }
    return true;
  }, [onUploadError]);
  ...
  const doUpload = async () => {
    ...
    try {
      ...
      const msg = typeof d==="string" ? d : Array.isArray(d) ? d.map(x=>x.msg).join(", ") : "Upload failed.";
  ...
  if (!file) return (
    <div role="button" tabIndex={0}
      ...
      style={{ border:`2px dashed ${drag?C.indigo:"rgba(255,255,255,.1)"}`,
      ...
      <p style={{ fontSize:13, color:C.textMuted }}>
        or <span style={{ color:C.info, fontWeight:600 }}>browse files</span> — PDF, PNG, JPG, WEBP up to 5 MB
      </p>
    </div>
  );
    ...
    <div style={{ ...cardBase, padding:20 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12,
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <p style={{ fontWeight:600, color:C.textPrimary, margin:0, fontSize:14,
            overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={file.name}>
            {file.name}
          </p>
          <p style={{ fontSize:12, color:C.textMuted, margin:0 }}>{fmt(file.size)}</p>
        </div>
        {!uploading && (
        ...
        <div style={{ marginBottom:14 }} aria-live="polite" aria-atomic="true">
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:C.textMuted, marginBottom:6 }}>
            <span style={{ display:"flex", alignItems:"center", gap:5 }}>
              <Loader2 size={11} color={C.info} aria-hidden="true"
                style={{ animation:"spin .7s linear infinite" }} />Uploading…
            </span>
            <span style={{ fontFamily:"'JetBrains Mono',monospace", color:C.info }}
      ...
      {!uploading && (
        <button type="button" onClick={doUpload}
          style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center",
            ...
            boxShadow:"0 6px 20px -4px rgba(99,102,241,.4)", marginTop:localErr?0:14 }}>
