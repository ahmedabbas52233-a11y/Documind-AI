...
export default function AnalysisPage() {
  ...
  const run = () => {
    ...
    fetch(`${API}/analyze/stream/${documentId}`, {
      ...
      .then(async (res) => {
        ...
        setPhase(p => {
          if (p === "loading") { setErrMsg("Connection closed before analysis completed. Please retry."); return "error"; }
          return p;
        });
      })
      .catch((err) => {
        if (err.name !== "AbortError") { setErrMsg(err.message || "Analysis failed. Please try again."); setPhase("error"); }
      });
  };
  ...
  const downloadReport = () => {
    ...
    const txt = [
      ...
      "", "SUMMARY", "─".repeat(40), parsed.analysis || "",
      "", "KEY POINTS", "─".repeat(40),
    ...
    <main id="main-content" style={{ maxWidth:1060, margin:"0 auto", padding:"36px 20px 72px" }}>
      ...
      <header style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <h1 style={{ fontSize:21, fontWeight:800, color:C.textPrimary, margin:0 }}>Analysis Results</h1>
            {filename && <p style={{ fontSize:12, color:C.textMuted, margin:0 }}>{filename}</p>}
          </div>
        </div>
        {phase === "done" && parsed && (
      ...
      {phase === "loading" && (
        <div style={{ ...cardBase, padding:"56px 24px", textAlign:"center" }}>
          ...
          <p style={{ fontSize:13, color:C.textMuted, marginBottom:24 }}>GPT-4o is reading your document…</p>
      ...
      {phase === "error" && (
        <div role="alert" style={{ ...cardBase, padding:"40px 24px", textAlign:"center" }}>
          ...
          <p style={{ fontSize:13, color:C.textMuted, marginBottom:22, maxWidth:360, margin:"0 auto 22px" }}>{errMsg}</p>
      ...
      {phase === "done" && parsed && (
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:12 }}>
            {[
              { icon: SentIcon,   color:sent.color, bg:sent.bg, border:sent.border, label:"Sentiment",      val:sent.label,                              valColor:sent.color },
              { icon: Star,       color:C.info,     bg:"rgba(99,102,241,.08)", border:"rgba(99,102,241,.15)", label:"Key Points",  val:(parsed.key_points||[]).length,   valColor:C.textPrimary },
              { icon: Lightbulb,  color:C.warn,     bg:"rgba(251,191,36,.07)", border:"rgba(251,191,36,.15)", label:"Actions",     val:(parsed.recommendations||[]).length, valColor:C.textPrimary },
              { icon: CheckCircle,color:C.success,  bg:"rgba(74,222,128,.07)", border:"rgba(74,222,128,.15)", label:"Status",      val:"Complete",                        valColor:C.success },
            ].map(({ icon:Icon, color, bg, border, label, val, valColor }) => (
              <div key={label} style={{ ...cardBase, padding:18, display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  <p style={{ fontSize:10, color:C.textMuted, fontWeight:600,
                    textTransform:"uppercase", letterSpacing:".07em", margin:0 }}>{label}</p>
                  <p style={{ fontSize:17, fontWeight:800, color:valColor, margin:0 }}>{val}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <style>{`
        ...
        @keyframes slide-prog{0%{transform:translateX(-100%)}100%{transform:translateX(250%)}}
      `}</style>
    </main>
  );
}
