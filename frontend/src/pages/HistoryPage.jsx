...
export default function HistoryPage() {
  ...
  if (!isAuthenticated) return (
    <main id="main-content" style={{ minHeight:"80vh", display:"flex",
      alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ ...cardBase, padding:48, maxWidth:340, width:"100%", textAlign:"center" }}>
        <div aria-hidden="true" style={{ width:48, height:48, borderRadius:12,
          background:"rgba(99,102,241,.09)", border:"1px solid rgba(99,102,241,.15)",
          display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px" }}>
          <History size={20} color={C.info} />
        </div>
        <h1 style={{ fontSize:20, fontWeight:800, color:C.textPrimary, marginBottom:8 }}>
          Sign in to view history
        </h1>
        <p style={{ fontSize:13, color:C.textMuted, marginBottom:24, lineHeight:1.6 }}>
          Create an account to save and revisit every analysis.
        </p>
  ...
  return (
    <main id="main-content" style={{ maxWidth:860, margin:"0 auto", padding:"40px 20px 72px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
        ...
        <header>
          <h1 style={{ fontSize:24, fontWeight:800, color:C.textPrimary, letterSpacing:"-.4px", margin:0 }}>
            Analysis History
          </h1>
          <p style={{ fontSize:13, color:C.textMuted, margin:0 }} aria-live="polite">
            {total} document{total!==1?"s":""} analysed
          </p>
        </header>
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <Link to="/upload" style={{ display:"inline-flex", alignItems:"center", gap:6,
            padding:"7px 14px", borderRadius:8, background:C.grad, color:"#fff",
      ...
      {loading && (
        <div aria-label="Loading…" aria-busy="true"
          ...
          {[...Array(5)].map((_,i) => (
            <div key={i} aria-hidden="true" style={{ ...cardBase, height:62,
      ...
      {!loading && !error && items.length === 0 && (
        <div style={{ ...cardBase, padding:"60px 24px", textAlign:"center" }}>
          <div aria-hidden="true" style={{ width:52, height:52, borderRadius:14,
            background:"rgba(255,255,255,.03)", border:`1px solid ${C.border}`,
            display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px" }}>
            <Clock size={22} color={C.textMuted} />
          </div>
          <h2 style={{ fontWeight:700, color:C.textPrimary, fontSize:18, marginBottom:6 }}>No history yet</h2>
          <p style={{ fontSize:13, color:C.textMuted, marginBottom:22 }}>Upload your first document to see it here.</p>
          <Link to="/upload" style={{ display:"inline-flex", alignItems:"center", gap:6,
            padding:"9px 20px", borderRadius:9, background:C.grad, color:"#fff",
            textDecoration:"none", fontWeight:600, fontSize:13 }}>
      ...
      {!loading && items.length > 0 && (
        <ul style={{ display:"flex", flexDirection:"column", gap:6, listStyle:"none", padding:0, margin:0 }}>
          {items.map(doc => (
            <li key={doc.id} style={{ ...cardBase, padding:"13px 16px",
              display:"flex", alignItems:"center", gap:12 }}>
              <div aria-hidden="true" style={{ width:34, height:34, borderRadius:8,
                background:"rgba(99,102,241,.07)", border:"1px solid rgba(99,102,241,.12)",
                display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <FileText size={14} color={C.info} />
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontWeight:600, color:C.textPrimary, fontSize:14, margin:0,
              ...
              <button type="button" onClick={()=>del(doc.id)} disabled={deleting===doc.id}
                ...
                style={{ background:"none", border:"none", cursor:"pointer",
                  color:C.textMuted, padding:5, borderRadius:6, display:"flex", flexShrink:0,
                  opacity:deleting===doc.id?.5:1 }}>
                {deleting===doc.id
      ...
      {pages > 1 && (
        <nav aria-label="Pagination" style={{ marginTop:22 }}>
          <ul style={{ display:"flex", alignItems:"center", justifyContent:"center",
            ...
            <li>
              <span style={{ fontSize:13, color:C.textMuted }} aria-live="polite">
                Page {page} of {pages}
              </span>
            </li>
            <li>
